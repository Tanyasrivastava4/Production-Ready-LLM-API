...
# -----------------------------
export ENV=GPU
export SALAD_PUBLIC_URL="https://feta-coriander-4fybgc189fwlr18m.salad.cloud:8001/v1/completions"
...
# -----------------------------
# 3️⃣ Start vLLM API server (internal, for container) in background
# -----------------------------
echo "Starting vLLM API server..."
python -m vllm.entrypoints.openai.api_server \
    --model mistralai/Mistral-7B-Instruct-v0.2 \
...
# -----------------------------
# 4️⃣ Start FastAPI server in background
# -----------------------------
echo "Starting FastAPI server..."
uvicorn main:app --host :: --port 8001 &
FASTAPI_PID=$!
